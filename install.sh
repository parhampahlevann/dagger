...
detect_default_ip() {
    local out=""
    for probe in 1.1.1.1 8.8.8.8 9.9.9.9; do
        out=$(ip route get "$probe" 2>/dev/null | awk '{for(i=1;i<=NF;i++) if($i=="src") print $(i+1)}' | head -1)
        [ -n "$out" ] && break
    done
...
write_netfix_helper() {
    cat > /usr/local/bin/daggerconnect-netfix.sh << 'NETFIX_EOF'
#!/bin/bash
# Auto-generated by DaggerConnect setup.sh — re-applies loose reverse-path
# filtering + ICMP rate-limit fixes on every service start (systemd ExecStartPre),
# so the fix survives reboots and new/renamed interfaces, not just the install run.
IP_BIN=$(command -v ip || echo /sbin/ip)
SYSCTL_BIN=$(command -v sysctl || echo /sbin/sysctl)

# Optional: delete a stale TUN device before the binary tries to recreate it
if [ -n "$1" ]; then
...
if [ -d /proc/sys/net/ipv6 ]; then
    "$SYSCTL_BIN" -w net.ipv6.conf.all.forwarding=1 >/dev/null 2>&1
    "$SYSCTL_BIN" -w net.ipv6.icmp.ratelimit=0 >/dev/null 2>&1
fi
exit 0
NETFIX_EOF
    chmod +x /usr/local/bin/daggerconnect-netfix.sh
}
...
apply_tun_sysctl() {
...

    {
        ...
        if [ -d /proc/sys/net/ipv6 ]; then
            ...
            echo "net.ipv6.icmp.ratelimit=0"
        fi
        if grep -qw bbr /proc/sys/net/ipv4/tcp_available_congestion_control 2>/dev/null; then
            echo "net.core.default_qdisc=fq"
            echo "net.ipv4.tcp_congestion_control=bbr"
        fi
    } > /etc/sysctl.d/99-daggerconnect.conf
...
install_service() {
    ...
    elif [ "$TRANSPORT" = "quantum" ]; then
        exec_pre="ExecStartPre=-/usr/local/bin/daggerconnect-netfix.sh"
    fi
